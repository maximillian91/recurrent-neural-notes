...
from lasagne.layers import ElemwiseSumLayer, ReshapeLayer
...
class GRUOutputInLayer(MergeLayer):
    r"""
    ...
    """
    def __init__(self, incoming, output_network, num_units,
        ...
        super(GRUOutputInLayer, self).__init__(incomings, **kwargs)
        ...
        # Generate the concatenation network for inputs
        print "DIAGNOSTICS"
        print num_inputs
        ...
        self.params.update(l_x.params)

        for l in get_all_layers(output_network):
            self.params.update(l.params)
        incomings.append(output_network)
        super(GRUOutputInLayer, self).__init__(incomings, **kwargs)
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        def step(input_n, output_previous, hid_previous, *args):
            # Compute W_{hr} h_{t - 1}, W_{hu} h_{t - 1}, and W_{hc} h_{t - 1}
            #print input_n
            #print previous
            #output_previous, hid_previous = previous
            zx = get_output(self.l_zx, {self.l_z_in:input_n, self.l_x_in:output_previous}, **kwargs)
            #zx = get_output(self.l_zx, {self.l_z_in:T.ones((2,5)), self.l_x_in:T.ones((2,20))}, **kwargs)

            hid_input = T.dot(hid_previous, W_hid_stacked)

            if self.grad_clipping:
                zx = theano.gradient.grad_clip(
                    zx, -self.grad_clipping, self.grad_clipping)
                hid_input = theano.gradient.grad_clip(
                    hid_input, -self.grad_clipping, self.grad_clipping)

            if not self.precompute_input:
                # Compute W_{xr}x_t + b_r, W_{xu}x_t + b_u, and W_{xc}x_t + b_c
                zx = T.dot(zx, W_in_stacked) + b_stacked

            # Reset and update gates
            resetgate = slice_w(hid_input, 0) + slice_w(zx, 0)
            updategate = slice_w(hid_input, 1) + slice_w(zx, 1)
            resetgate = self.nonlinearity_resetgate(resetgate)
            updategate = self.nonlinearity_updategate(updategate)

            # Compute W_{xc}x_t + r_t \odot (W_{hc} h_{t - 1})
            hidden_update_in = slice_w(zx, 2)
        ...
        x_init = T.zeros((num_batch, self.output_network.output_shape[-1]))

        # The hidden-to-hidden weight matrix is always used in step
        non_seqs = get_all_params(self.l_zx) + get_all_params(self.output_network) + [W_hid_stacked]
        ...
        if self.unroll_scan:
            ...
            x_out, hid_out = unroll_scan(
                fn=step_fun,
                sequences=sequences,
                outputs_info=[x_init, hid_init],
...
if __name__ == '__main__':
    ...
    NUM_Z = 5
    NUM_SEQ = 45
    out_net = InputLayer((None, NUM_UNITS_DEC))
    out_net = DenseLayer(out_net, 20, nonlinearity=nonlinearities.softmax)

    l_in = InputLayer((None, NUM_Z))
    l_in_rep = RepeatLayer(l_in, n=NUM_SEQ)
    l_dec = GRUOutputInLayer(l_in_rep, out_net, num_units=NUM_UNITS_DEC)


    z = np.random.randint(0, 10, size=(2, NUM_Z)).astype('float32')
    z_sym = T.matrix('z').astype('float32')
    print lasagne.layers.get_output(l_dec, inputs={l_in: z_sym}).eval(
        {z_sym: z}).shape

    #Get parameters of both encoder and decoder
    all_parameters = lasagne.layers.get_all_params([l_dec], trainable=True)
